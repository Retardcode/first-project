...
<style>
    .stApp {
        background: radial-gradient(circle at 10% 20%, rgb(10, 11, 24) 0%, rgb(4, 5, 12) 90%);
        color: #e0e6ed;
    ...
    .title-glow {
        ...
        letter-spacing: 3px;
        text-shadow: 0px 0px 25px rgba(121, 40, 202, 0.5);
        margin-bottom: 0px;
    }
    .subtitle-glow {
        color: #00f2fe;
    ...
    .stButton>button:hover {
        transform: translateY(-2px); box-shadow: 0 6px 25px rgba(0, 242, 254, 0.6);
...
# Calculate Dynamic Risk Score based on Table Activity
risk_score = 0
...
def render_home():
    ...
    st.markdown('<p class="subtitle-glow">Multi-Table Neural Database</p>', unsafe_allow_html=True)
    
    col1, col2, col3, col4 = st.columns(4)
    col1.metric("Total Data Nodes", f"{total_docs}", "Vectors across 4 Tables", delta_color="off")
    col2.metric("System Readiness", f"{risk_score}%", "Operational Health")
    col3.metric("Auto-Routing", "Active", "Dynamic SQL Tagging", delta_color="normal")
    col4.metric("LLM Search Depth", "Maximal", "Threshold: 0.3", delta_color="normal")

    if df_stats.empty:
        st.markdown('<div class="empty-state">SYSTEM EMPTY: ALL 4 SUPABASE TABLES AWAIT DATA</div>', unsafe_allow_html=True)
    else:
        st.markdown("### 🌐 Global Data Distribution Nodes")
        df_map = pd.DataFrame({
            "lat": np.random.randn(total_docs * 3) / 50 + 37.76,
            "lon": np.random.randn(total_docs * 3) / 50 - 122.4,
            "size": np.random.rand(total_docs * 3) * 100,
        })
        st.map(df_map, color="#00f2fe", size="size")
...
def render_risk():
    ...
    st.markdown('<p class="subtitle-glow">Relational Vector Analytics</p>', unsafe_allow_html=True)
    ...
    with col1:
        ...
        fig_gauge = go.Figure(go.Indicator(
            mode = "gauge+number",
            value = risk_score,
            title = {'text': "Multi-Table Policy Readiness", 'font': {'color': 'white'}},
            gauge = {
    ...
    with col2:
        st.markdown("### 🗂️ Autonomous Neural Routing Density")
        category_counts = df_stats['category'].value_counts().reset_index()
        category_counts.columns = ['Category (SQL Table)', 'Vector Count']
        fig2 = go.Figure(data=[go.Pie(labels=category_counts['Category (SQL Table)'], values=category_counts['Vector Count'], hole=.5)])
        fig2.update_traces(
...
def render_upload():
    ...
    st.markdown('<p class="subtitle-glow">Autonomous Multi-Table Router</p>', unsafe_allow_html=True)
    
    st.info("The RAG Engine will read your file and execute an SQL INSERT to the correct database table automatically. Files with no tags will be sent to General Knowledge.")
    
    with st.expander("📥 1. Generate Demo Payloads (Click to Download)"):
        st.write("Save these to your computer, then drag them into the uploader below to watch the Auto-Router work.")
        ...
        # This 5th file has NO tag, testing the fallback mechanism.
        u_text = "TITLE: Cafeteria Menu Update\nStarting next week, the Nexus Cafeteria will serve Sushi on Thursdays. Please update your meal preferences."
        ...
        c5.download_button("Untagged File", u_text, file_name="demo_untagged.txt")
...
def render_chat():
    ...
    if prompt := st.chat_input("Enter complex query command..."):
        ...
        with st.chat_message("assistant"):
            ...
            with st.spinner("Executing UNION ALL search across 4 Database Tables..."):
